...
# 1. Blueprint tanımlaması (admin_app yerine admin_bp)
admin_bp = Blueprint('admin', __name__, url_prefix='/admin')
ADMIN_SIFRE = "1234"

# Veritabanı fonksiyonun (Render ile uyumlu)
def veritabanina_baglan():
    return psycopg2.connect(os.environ.get('DATABASE_URL'), sslmode='require')

# Senin orijinal sayfa yapın
...
# 2. admin_app yerine admin_bp ile yönlendirme
@admin_bp.before_request
def guvenlik():
    if request.endpoint and 'admin' in request.endpoint and 'admin.admin_giris' not in request.endpoint:
        if not session.get('giris'): return redirect(url_for('admin.admin_giris'))
...
def admin_anasayfa():
    ...
    # Linkleri url_for('admin.fonksiyon') şeklinde güncelledim
    tablo = "".join([f"<tr><td>{u[1]}</td><td>{u[2]} TL</td><td>{u[3]}</td><td><a href='{url_for('admin.urun_duzenle', id=u[0])}'>✏️</a> | <a href='{url_for('admin.urun_sil', id=u[0])}' style='color:#ff4d4d;'>❌</a></td></tr>" for u in urunler])
    return sayfa_yapisi(f"<h2>🛠️ Yönetim Paneli</h2><form method='GET' action='{url_for('admin.admin_anasayfa')}'><input name='q' placeholder='Ürün ara...' value='{arama}'></form><a href='{url_for('admin.urun_ekle')}' class='btn'>+ Yeni Ürün Ekle</a><table><tr><th>Ürün</th><th>Fiyat</th><th>Kat.</th><th>İşlem</th></tr>{tablo}</table><a href='{url_for('admin.admin_cikis')}' class='btn' style='background:#555;'>Oturumu Kapat</a>")
